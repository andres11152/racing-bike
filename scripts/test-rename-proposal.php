<?php
/**
 * Propuesta completa de renombramiento de productos al formato:
...
// Mapeo exhaustivo y preciso de cada producto por su ID
$mapping = [
    // --- CASCOS Y PROTECCIÓN ---
    2056 => [
...
echo "=== TABLA DE PROPUESTA DE RENOMBRAMIENTO ===" . PHP_EOL . PHP_EOL;
...
foreach ($products as $p) {
    ...
    if (isset($mapping[$id])) {
        $m = $mapping[$id];
        $proposed = "{$m['tipo']} {$m['marca']} {$m['modelo']}";
        echo "ID {$id} [{$p->get_status()}]\n";
        echo "  Actual:   {$current}\n";
        echo "  Propuesto: {$proposed}\n";
        echo "  Partes:   [{$m['tipo']}] + [{$m['marca']}] + [{$m['modelo']}]\n\n";
    } else {
        $unmapped[] = $id . ": " . $current;
    }
}
...
if (!empty($unmapped)) {
    echo "¡ATENCIÓN! Productos sin mapear:\n" . implode("\n", $unmapped) . "\n";
} else {
    echo "¡ÉXITO! Los 59 productos tienen mapeo 100% definido.\n";
}
