<?php
/**
 * Test de unicidad absoluta con diccionario completo de 77 colores.
 */
...
foreach ($products as $p) {
    ...
    if (isset($allSkus[$parentSku])) {
        $duplicates[] = "PADRE DUPLICADO: {$parentSku}";
    }
    ...
    if ($p->is_type('variable')) {
        foreach ($p->get_children() as $childId) {
            ...
            if (isset($allSkus[$varSku])) {
                $duplicates[] = "VARIACIÓN DUPLICADA: {$varSku} (Var {$childId} con {$allSkus[$varSku]})";
            }
...
echo "TOTAL PRODUCTOS: 58\n";
echo "TOTAL SKUS ÚNICOS: " . count($allSkus) . "\n";
echo "DUPLICADOS: " . count($duplicates) . "\n\n";

if (count($duplicates) === 0) {
    echo "¡100% ÉXITO! Todos los " . count($allSkus) . " SKUs son absolutamente únicos, ordenados y legibles.\n";
} else {
